...
std::vector<int> MTMSolver::solve() {
	...
	while (heuristic) {
		...
		if (L > z) {
			...
			z = L;
			for (j = 0; j < n; j++) // n Number of items
				x[j] = -1;  //x Current best solution (knapsack for each item)
			for (k = 0; k < m; k++) // // Number of knapsacks
				for (j = 0; j < n; j++)
					x[j] = (xh[k*n + j] == 1) ? k : x[j];  // xh?
			for (k = i; k < m; k++)
				for (j = 0; j < n; j++)
					if (xt[k*n + j] == 1)
						x[j] = k;
			...
			if (z == Ur) {  // Ur Upper bound at root node
				break; // stop search
			}
			
			// Best solution has been found for the current node
			if (z == U) {// int U; // Upper bound for current solution
				backtrack = true;
		...
		if (update) {
			...
			while (i < m - 1) {
				...
				I = {};
				for (l = 0; l < n; l++)
					if (xt[i*n + l] == 1) // xt  Latest solution calculated in lower bound
						I.push_back(l);
				...
				while (I.size() > 0) {
					...
					S[i].push_back(j); // // Unlabeled (=assigned) items for each knapsack 当前itemj装箱 i表示服务器，j表示虚拟机
					xh[i*n + j] = 1;
					cr[i] -= w[j];  // Knapsack residual capacities for current solution 服务器剩余容量，cpu&mem
					ph += p[j];     // ph// Total profit of current solution 目前所装的总价值
					jhuse[j] = 1;   // jhuse// Whether an item is assigned to a knapsack
					Uj[j] = U;      // 
